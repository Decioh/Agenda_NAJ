...
@section('title', 'Adicionando nova parte')
...
<div id="search-container" class="col-md-12 justify-content-center">
    ...
    <form action="{{route('agenda.nova_parte',$agenda_id)}}" method="GET">
        @csrf
...
@foreach ($assistidos as $assistido)
        ...
        <tbody>
            <tr>
                ...
                <td>{{$tel}}</td>
                <td>
                    <a href="{{route('agenda.join',['id'=>$assistido->id,'agenda_id'=>$agenda_id])}}"class="btn btn-success btn-sm"> Adicionar a conciliação</a>
                </td>
            </tr>
        </tbody>
